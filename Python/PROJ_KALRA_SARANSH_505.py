...
#
#METHOD:
#function created to calculate calorie requirement in which calculate bmr according to the whether a person is male or female
...
#file diet_data.csv opened for writing in append mode and  for reading in read mode
#open csv reader to read from the file 
#attached all the records one by one into a single list
...
#created a list "input_lst" to store the new record to be entered to the file
#declared a list "lst1" to hold a record read from a file to print it on screen
#created counter to count till 7 i.e the length of a record so that all records can be printed separately
#Display the main menu showing
#option 1 print all the existing data read from the file
#iterate through the whole list after 7 terms i.e. a record, line changes so that new record prints in newline
#if option is 2 prompt the user for the data of a new client to be input into the file as a new record
#loops required to check all data validations, print appropriate messages and break the loop when all of the inputs are valid
#and the bmr is not negative ,then only its a valid record.
#validate bmr to be positive for male anf female ,if bmr comes out to be negative give error message and if its positive then data is valid
#if valid then break out of the final loop after writing the data to the file.
#If everything is valid then write the data in the list input_lst'keeping records in program and to the file                       
#if option is 3 give the user chance to select a client and then select an action for it from another sub menu "Food menu".
#number of variables declared to store and print calorie and fat required by patient
#A loop runs until the record is not found
#validates the last name entered by the user to not contain anything other than characters    
#for loop to iterate through the records to find the last name
#and have condition to not count the records whose first name is the last name searched as one index before it would have some number,
#counters to keep track of all records matched with last name, also if client not found.
#if client found check and print if we have more than one records found with the same last name  and ask user to choose the one he/she wants.
#validation done on selection and print error messages.
#Loop to go for calculation of calories and fat requirement with printing of sub menu for daily calorie , breakfast, lunch and dinner and recommendation.
#if option1 is 1 display the fat grams needed, daily calorie need and the daily fat calorie need storing calorie requirement and show the bmr calculated.
#if option1 is 2 then provide the user a menu for choosing a food menu for the client select
#ask the user if the client wishes to lose weight ,if yes the calorie requirement becomes 90%,if not the calorie requirement remains as calculated for the record.
#loop required to print list of food items with their calorie and fat,
#take input from the user to enter choices for food options so that proper the calorie requirement should be met.
#corresponding to the number in the menu whichever option the user enters, the corresponding food item
#gets added to the food list and the calories and fatgram it provides get added to variables and print messages.
#if calorie requirement is met or not met, proper messages are printed.
#check if the user list of food not final print and calorie need already met , give warning about the calorie requirement is already already fulfilled
#adding anything else exceeds the calorie need.      
#if the option1 chosen is 3 print the calorie requirement and fat gram requiredment if the client needs to lose weight needed daily.
#if option is 4 ask the user which record he wants to edit
#validate the last name entered by the user to not contain anything other than characters
...
#validate the serial number entered by the user
#made loops to validate each entry (refer option 1 same as it was) like first and last name should be strings height,weight,age should be float and activity should be integer
...
#if for male or female sex the bmr comes out to be negative error message given and asked to input whole data again
...
#iterating through the elements of the list, except the deleted record everything else gets written to a new list
#closed the file which was opened in append mode
#opened the file in write mode so it wipes all data and write the file again with the edited record
#declare csv writer again as it file mode has been changed
...
#if any other option give error message
...
for row in breader:
    #attaching all the records one by one into a single list
...
while(valid=='0'):
    ...
    if(option=='1'):
        ...
        print("-------------------------------------------------")
        #iterating through the whole list after 7 terms i.e. a record, line changes so that new record prints in newline
        ...
        #will only break the the loop when all of the inputs are valid and the bmr is not negative then only its a valid record
        while(valid1=='1'):
            val1='0'
            val2='0'
            val3='0'
            ...
            #validate bmr to be positive for female
            elif(sex=='female' or sex=='Female' or sex=='FEMALE'):
                #if bmr comes out to be negative give error message
            ...
            #if valid1 is zero means everything is valid then write the data in the list keeping record in program and to the file
            if(valid1=='0'):
        ...
        found='0'
        #variable acting as a temp variable to store indexes of records found during the iteration
        index_found=-1
        ...
        while(found=='0'):
            ...
            if(found=='0'):
                ...
                while(valid2=='0'):
                    ...
                    if(option1=='1'):
                        #store calorie requirement in cal_req by passing the sex, weight,height,age,activity of the record selected
        ...
        while(found=='0'):
            ...
            if(found=='0'):
                ...
                while(valid1=='1'):
                    ...
                    #made loops to validate each entry (refer option 1 same as it was) like first and last name should be strings height,weight,age should be float and activity
                    #should be integer
                    #ensured height,weight,age,activity are in range
                    #show recorded value
                    #ask the user if he wants to change this value
